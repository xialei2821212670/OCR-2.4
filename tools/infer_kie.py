...
def draw_kie_result(batch, node, idx_to_cls, count):
    ...
    image_bg = np.zeros(img.shape, dtype="uint8")
    for i, box in enumerate(boxes):
        ...
        image_polylines = cv2.polylines(
            image_bg, [Pts.reshape((-1, 1, 2))],
            True,
            color=(255, 255, 0),
            thickness=1)
        # cv2.imshow('image_polylines', image_polylines)
        # # 封闭区域内填充白色
        # mask = cv2.fillPoly(image_polylines, [Pts.reshape((-1, 1, 2))], (255, 255, 255))
        #
        # cv2.imshow('image_polylines2', mask)
        # # 第三步 将连接起来的区域对应的数组和原图对应位置按位相与，这样就截取到了需要的区域了
        # ROI = cv2.bitwise_and(img, mask,mask=No)
        # cv2.imwrite(r'D:\python\PaddleOCR-release-2.4\output\sdmgr_kie\kie_results\12.jpg', ROI)
        # cv2.imshow('mask', ROI)
        ...
        images =img[y_min:y_max,x_min:x_max]

        # cv2.imwrite(r'D:\python\PaddleOCR-release-2.4\output\sdmgr_kie\kie_results\12.jpg', images)
        cv2.imshow("as",images)
        cv2.waitKey(0)
