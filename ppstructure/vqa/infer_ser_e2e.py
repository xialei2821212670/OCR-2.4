...
if __name__ == "__main__":
    ...
    for idx, img_path in enumerate(infer_imgs):
        ...
        for i in range(len(result)):
            # if result[i]['pred'] =='O':
            #     print("农村地承包经营权证编码:", result[i]['text'])
            if index > 7:
                break
            if result[i]['pred'] == 'QUESTION':
                index = index + 1
                key = result[i]['text']
                v = ''
                pred = result[i + 1]['pred']
                if pred == 'ANSWER':
                    v = result[i + 1]['text']
                dic_res[key] = v

        print(dic_res)
        all_res.append(dic_res)

        #     if time == 1:
        #         print("发包方名称:", result[i + 1]['text'])
        #     if time == 2:
        #         print("承包方代表姓名:", result[i + 1]['text'])
        #     if time == 3:
        #         print("身份证号码:", result[i + 1]['text'])
        #     if time == 4:
        #         print("承包方式:", result[i + 1]['text'])
        #     if time == 5:
        #         print("承包方住址:", result[i + 1]['text'])
        #     if time == 6:
        #         print("邮政编码:", result[i + 1]['text'])
        #     if time == 8:  # 土地承保合同编号
        #         print("土地承保合同编号:", result[i + 1]['text'])
        # if result[i]['pred'] == 'ANSWER':
        #     index = index + 1
        #     if index == 7:
        #         if (result[i + 1]['text']).isdigit() and len(result[i + 1]['text']) <= 12:
        #             print("联系电话", result[i + 1]['text'])
